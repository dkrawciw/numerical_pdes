...
# n = 50
n_list = range(10,110,5)
error_2norm = []
error_infnorm = []

for n in tqdm(n_list, desc="Immersed BVP with Different Grid Sizes"):
    x_range = (0, 2*np.pi)
    y_range = (0, 2*np.pi)
    ...
    exact_soln = np.cos(2*X_ext) * np.sin(2*Y_ext)
...
plt.ylabel("Error")
plt.title("Error Convergence of the Immersed Boundary Value\nProblem as the number of Grid Points Increases")
plt.legend()
