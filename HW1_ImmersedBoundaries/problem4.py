...
x_ib_1 = np.pi/2 + np.sin(thetas)*np.cos(thetas)
y_ib_1 = 3*np.pi/2 + np.sin(thetas)
x_ib_2 = 3*np.pi/2 + np.sin(thetas)*np.cos(2*thetas)
y_ib_2 = 3*np.pi/2 + np.sin(2*thetas)*np.sin(thetas)
x_ib_3 = np.pi + np.sin(2*thetas)*np.cos(thetas)
y_ib_3 = np.pi/2 + np.sin(2*thetas)*np.sin(thetas)
x_ib_4 = np.pi + 1/2*np.cos(thetas)
y_ib_4 = np.pi + 1/2*np.sin(thetas)

x_ib = np.concatenate([x_ib_1,x_ib_2,x_ib_3,x_ib_4])
y_ib = np.concatenate([y_ib_1,y_ib_2,y_ib_3,y_ib_4])
...
plt.xlabel(r'$x$')
plt.ylabel(r'$y$')
plt.title('*Almost* Minimal Surface with Interesting\nShapes Creating the Immersed Boundary', fontsize=20)
plt.tight_layout()

# plt.show()
plt.savefig("output/problem4.svg")